...
def load_Fasta_file():
    file_path = filedialog.askopenfilename(title="chuse FASTA file", filetypes=[("FASTA files""*fasta *.fa"), ("All files", "*.*")])
    if not file_path:
        return
    try: 
...
def plot_distr_result(results):
    for widget in graph_frame.winfo_children():
        widget.destroy()
    labels = ['A','T','G','C']
    sizes = [results['count_a'], results['count_t'], results['count_g'], results['count_c']]
