...
class BioApp:
    ...
    def _create_palindrome_components(self):
        ...
        min_entry = ttk.Entry(params_frame,
                              textvariable=self.palindrome_min_len,
                              width=5)
        min_entry.pack(side=tk.LEFT, padx=5)

        ttk.Label(params_frame, text="Max length:").pack(side=tk.LEFT)
        max_entry = ttk.Entry(params_frame,
                              textvariable=self.palindrome_max_len,
                              width=5)
    ...
    def _create_codon_heatmap(self):
        ...
        pivot_df = df.pivot_table(index='Amino Acid',
                                  columns='Codon',
                                  values='Frequency',
                                  fill_value=0,
                                  aggfunc='sum')
        ...
        # Office Settings
        ax.set_title("Codon Usage Heatmap (Grouped by Amino Acid)",
                     pad=20,
                     fontsize=14)
    ...
    def _update_palindrome_display(self):
        ...
        if not self._current_result or not self._current_result._data[
                'palindromes']:
        ...
        # Adding data
        for idx, palindrome in enumerate(
                self._current_result._data['palindromes']):
            tags = ('even', ) if idx % 2 == 0 else ('odd', )
            tree.insert('',
                        tk.END,
                        values=(palindrome['position'], palindrome['length'],
                                palindrome['sequence']),
                        tags=tags)
