    ...
    def _check_length(self):
        if len(self) < self.MIN_LENGTH:
            raise SequenceValidationError(
                f"Minimum sequence length: {self.MIN_LENGTH}")
...
class DNA(__Sequence):
    ...
    def find_codons(self, codon_type: str) -> list[int]:
        ...
        return [
            i for i in range(0,
                             len(self) - 2, 3)
            if self.sequence[i:i + 3] in targets
        ]
    ...
    def reverse_complement(self) -> str:
        ...
        return ''.join(
            complement.get(base, '') for base in reversed(self.sequence))

    def find_palindromic_sequences(self,
                                   min_len: int = 4,
        ...
        for length in range(max_len, min_len - 1,
                            -1):  # Come from greater to smaller
        ...
        results.sort(
            key=lambda x:
            (-x['length'], x['start']))  # Sorting in length and position

        occupied = []
        for item in results:
            # We check if the current palindrome is intersected with the already selected
            overlap = any(item['start'] < existing['end']
